...
var supabase = (0, import_supabase_js.createClient)(supabaseUrl, serviceKey, { auth: { persistSession: false } });
...
function verify(token) {
  ...
  const expected = import_node_crypto.default.createHmac("sha256", SESSION_SECRET).update(body).digest("base64url");
...
async function handler(event) {
  ...
  try {
    ...
    if (event.httpMethod === "GET") {
      ...
      const id = params.id;
      if (id) {
        const { data: data2, error: error2 } = await supabase.from("trips").select("*").eq("id", id).single();
        if (error2) return json(404, { error: error2.message }, event);
        return json(200, data2, event);
      }
      ...
      if (upcoming && !all) q = q.gte("date_time", (/* @__PURE__ */ new Date()).toISOString());
